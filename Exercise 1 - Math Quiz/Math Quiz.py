...
class QuizApp:
    ...
    def play_background_music(self, music_file="chill.mp3"):
        """Loads and plays music in a continuous loop at a low volume."""
        try:
            # FIX: Construct the full path using self.script_dir and the local filename
    ...
    def end_fullscreen(self, event=None):
        ...
        # Stop the music when exiting fullscreen
    ...
    def set_image_background(self, image_path):
        ...
        try:
            ...
            # If file is not found, we avoid the messagebox and use the fallback
    ...
    def decide_operation(self): return random.choice(['+', '-'])

    def is_correct(self, user_answer):
        try: return int(user_answer) == self.current_problem['answer']
    ...
    def quit_game(self):
        if messagebox.askyesno("Quit Game", "Are you sure you want to quit the current quiz? Your score will be lost."):
    ...
    def display_menu(self):
        ...
        title_label = tk.Label(menu_frame, text="🎯 MATH MASTER", font=self.header_font, 
                     bg=self.BG_LIGHT, fg=self.ACCENT_YELLOW)
        title_label.pack(pady=(0, 10))
        
        subtitle_label = tk.Label(menu_frame, text="Select Difficulty Level", 
                                 font=("Segoe UI", 14), bg=self.BG_LIGHT, fg=self.TEXT_COLOR)
